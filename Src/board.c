...
float convert_reading_to_charge_current(const uint16_t adc_acs712_reading, const uint16_t adc_acs712_offset) {
		...
		ads_current_v = 2.0f*adc_get_voltage_raw(ads_current_v);
		//return acs712_voltage/ACS_SENSITIVITY;
	//	return (float)1000.0f*ads_current_v*0.002745f;
	return (float)1000.0f*ads_current_v/ACS_SENSITIVITY;
}
...
float board_adc_to_battery_volt(const uint16_t adc_battery_reading) {
	raw_batt_volt = adc_get_voltage_raw(adc_battery_reading);
	return adc_get_voltage_raw(adc_battery_reading)*8.28f/1.88f;
}
...
void board_step(const uint32_t period_ms) {
		...
		adc_current_avr = average_filter(adc_current_buff, ADC_CURRENT_SIZE, adcVal[0], &adc_current_cnt);
